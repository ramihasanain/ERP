...
const TrialBalance = () => {
    ...
    const isBalanceStatusKnown = !trialBalanceQuery.isLoading && !trialBalanceQuery.isError;
    const totalAccountPages = Math.max(1, Math.ceil(trialBalance.accounts.length / ACCOUNT_PAGE_SIZE));
    const safeCurrentPage = Math.min(currentPage, totalAccountPages);
    const paginatedAccounts = useMemo(() => {
        const startIndex = (safeCurrentPage - 1) * ACCOUNT_PAGE_SIZE;
        return trialBalance.accounts.slice(startIndex, startIndex + ACCOUNT_PAGE_SIZE);
    ...
    const handleExportExcel = () => {
        ...
        if (trialBalanceQuery.isError) {
            toast.error('Could not export trial balance. Please reload the data first.');
            return;
        }
        ...
        const rows = [
            ...
            [],
            ['Total', '', formatExcelAmount(trialBalance.totalDebit), formatExcelAmount(trialBalance.totalCredit)],
        ];
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', minWidth: 0 }}>
                    ...
                    <div>
                        ...
                        <p style={{ color: 'var(--color-text-secondary)' }}>As of {formatDateLabel(trialBalance.asOf || filters.asOf)}</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '1rem' }} className="shrink-0">
                    <Button
                        ...
                        disabled={trialBalanceQuery.isLoading || trialBalanceQuery.isError || trialBalance.accounts.length === 0}
            ...
            <Card className="padding-lg">
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
                    ...
                    <div style={filterFieldStyle}>
                        ...
                        >
                            {accountTypesQuery.isLoading ? (
                                <option value={filters.accountType}>Loading account types...</option>
                            ) : accountTypes.length === 0 ? (
                                <option value={filters.accountType}>{filters.accountType}</option>
                            ) : (
                    ...
                    <div style={filterFieldStyle}>
                        ...
                        <select
                            ...
                            onChange={(event) => updateFilter('includeZeroAccounts', event.target.value)}
                            style={selectStyle}
                            className="cursor-pointer"
                        >
                ...
                <div style={{
                    ...
                    gap: '0.5rem',
                    marginBottom: '1.5rem',
                    color: isBalanceStatusKnown && trialBalance.isBalanced ? 'var(--color-success)' : 'var(--color-warning)',
                    fontWeight: 600
                }}>
                    {isBalanceStatusKnown && trialBalance.isBalanced ? <CheckCircle size={20} /> : <AlertCircle size={20} />}
                ...
                {trialBalanceQuery.isLoading ? (
                    ...
                    <div style={{ padding: '1rem 0', color: 'var(--color-error)' }}>
                        ...
                        <div style={{ marginTop: '1rem' }}>
                            <Button variant="outline" onClick={() => trialBalanceQuery.refetch()} className="cursor-pointer">
                                Retry
                            </Button>
                        </div>
                    </div>
                            ...
                            <table style={tableStyle}>
                                ...
                                <tbody>
                                    {trialBalance.accounts.length === 0 ? (
                                        <tr>
                                            <td colSpan={4} style={{ ...tableCellStyle, color: 'var(--color-text-secondary)' }}>
                                                No accounts found for the selected filters.
                                            </td>
                                        </tr>
                                    ) : (
                                        paginatedAccounts.map((account) => (
                                            <tr key={account.account_id || account.code} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                                <td style={{ ...tableCellStyle, fontFamily: 'var(--font-mono)' }}>{account.code || '-'}</td>
                                                <td style={tableCellStyle}>{account.name || '-'}</td>
                                                <td style={tableAmountCellStyle}>{formatAmount(account.debit)}</td>
                                                <td style={tableAmountCellStyle}>{formatAmount(account.credit)}</td>
                                            </tr>
                                        ))
                                    )}
                                    ...
                                    <tr style={{ fontWeight: 700, background: 'var(--color-bg-table-header)', color: 'var(--color-text-main)' }}>
                                        ...
                                        <td style={totalAmountCellStyle}>{formatAmount(trialBalance.totalDebit)}</td>
                                        <td style={totalAmountCellStyle}>{formatAmount(trialBalance.totalCredit)}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
