...
const PermissionsEmployeeAssignmentsTab = ({ enabled }) => {
    ...
    const assignmentsQuery = useCustomQuery('/api/roles/assignments/', ['permissions', 'role-assignments'], {
        enabled,
        staleTime: Infinity,
    ...
    const assignmentRows = useMemo(() => assignmentsListFromPayload(assignmentsQuery.data), [assignmentsQuery.data]);
    const roleOptions = useMemo(() => rolesListFromPayload(rolesQuery.data), [rolesQuery.data]);
    ...
    const assignRoleMutation = useMutation({
        ...
        onSuccess: async () => {
            await Promise.all([
                queryClient.invalidateQueries({ queryKey: ['permissions', 'role-assignments'] }),
    ...
    const assignmentsTotalPages = Math.max(1, Math.ceil(assignmentRows.length / PAGE_SIZE));
    const assignmentsCurrentPage = Math.min(assignmentsPage, assignmentsTotalPages);
    ...
    const assignmentIdForRow = (row) => row.assignment_id ?? row.assignmentId ?? row.id;
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '640px' }}>
                        <thead>
                            <tr style={{ background: 'var(--color-bg-table-header)', fontSize: '0.85rem', color: 'var(--color-text-muted)' }}>
                                ...
                                <th style={{ padding: '12px 1.5rem', textAlign: 'right' }}>Modules</th>
                            </tr>
                        </thead>
                        <tbody>
                            {paginatedAssignments.map((row) => {
                                const deptLabel = row.department_name ?? '—';
                                const currentRoleId = row.role?.id != null ? String(row.role.id) : '';
                                const assignmentId = assignmentIdForRow(row);
                                const rowKey = assignmentId != null && assignmentId !== ''
                                    ? String(assignmentId)
                                    : `emp-${row.id}`;
                                const rowBusy = assignRoleMutation.isPending
                                ...
                                return (
                                    <tr key={rowKey} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                        <td style={{ padding: '12px 1.5rem' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>{row.id}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td style={{ padding: '12px 1rem', verticalAlign: 'middle' }}>
                                            <select
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    assignRoleMutation.mutate({ assignmentId, roleId: nextId });
                                                }}
                                            >
                                                {!currentRoleId && (
                                        ...
                                        <td style={{ padding: '12px 1.5rem', textAlign: 'right' }}>
                                            <span style={{
                                                ...
                                                padding: '3px 10px',
                                                borderRadius: '10px',
                                                background: (row.modules_count ?? 0) > 0
                                                    ? 'color-mix(in srgb, var(--color-primary-600) 14%, var(--color-bg-card))'
                                                    : 'var(--color-bg-subtle)',
                                                color: (row.modules_count ?? 0) > 0 ? 'var(--color-primary-600)' : 'var(--color-text-secondary)',
                                                fontWeight: 600,
                                            }}
                                            >
                                                {row.modules_count ?? 0} module{(row.modules_count ?? 0) !== 1 ? 's' : ''}
                                            </span>
                                        </td>
                                    </tr>
                                );
