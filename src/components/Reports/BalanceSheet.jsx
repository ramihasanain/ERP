    ...
    if (isError || !data) {
        return (
            <Card className="padding-lg">
                <p style={{ color: 'var(--color-error)' }}>{t('loadFailed.balanceSheet')}</p>
            </Card>
        );
    }
        ...
        <div ref={printableRef} className="printable-area" style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
            <div className="no-print" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div>
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 700 }}>{t('balanceSheet')}</h1>
                    <p style={{ color: 'var(--color-text-secondary)' }}>{t('balanceSheetSubtitle')}</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.55rem' }}>
                        <span style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>{t('asOf')}: {asOfDate}</span>
            ...
            <Card className="padding-lg">
                <div style={{ textAlign: 'center', marginBottom: '2rem', borderBottom: '1px solid var(--color-border)', paddingBottom: '1rem' }}>
                    ...
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>{t('balanceSheetReport.title')}</h3>
                    <p style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)' }}>{t('balanceSheetAsOf', { date: data.as_of })}</p>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
                        <h3 style={{ fontSize: '1.2rem', fontWeight: 700, borderBottom: '2px solid var(--color-primary-600)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>{t('balanceSheetReport.assets')}</h3>
                        ...
                        <div style={{ marginTop: 'auto' }}>
                            ...
                            <GrandTotalRow label={t('balanceSheetReport.totalAssets')} amount={data?.assets?.total || 0} formatter={formatMoney} />
                        </div>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
                        <h3 style={{ fontSize: '1.2rem', fontWeight: 700, borderBottom: '2px solid var(--color-error)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>{t('balanceSheetReport.liabilities')} & {t('balanceSheetReport.equity')}</h3>
                        ...
                        <SectionBlock
                            title={t('balanceSheetReport.equity')}
                            lines={equityLines}
                            total={data?.liabilities_and_equity?.equity?.total || 0}
                            formatMoney={formatMoney}
                        />

                        <Row label={t('balanceSheetReport.netIncomeCurrentYear')} amount={data?.liabilities_and_equity?.net_income_current_year || 0} formatter={formatMoney} highlight />

                        <div style={{ marginTop: 'auto' }}>
                            <div style={{ height: '1.25rem' }} />
                            <GrandTotalRow
                                label={t('balanceSheetReport.totalLiabEquity')}
...
const SectionBlock = ({ title, lines = [], total = 0, formatMoney }) => {
    ...
    <div style={{ marginBottom: '1rem' }}>
        ...
        {lines.length === 0 ? (
            <div style={{ paddingLeft: '1rem', color: 'var(--color-text-muted)', fontStyle: 'italic' }}>{t('pnl.noLines')}</div>
        ) : (
            lines.map((line) => (
                <Row
        ...
        <TotalRow label={t('balanceSheetReport.total', { section: title })} amount={total} formatter={formatMoney} />
    </div>
    );
};
