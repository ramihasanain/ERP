...
const MyRequests = () => {
    ...
    const deleteRequestMutation = useCustomRemove(resolveDeleteUrl, [['employee-my-requests']]);

    const createLeaveMutation = useCustomPost('/api/hr/employees/leaves/create/', [
        ['employee-my-requests'],
        ['hr-employee-dashboard'],
    ]);
    const uploadDocumentMutation = useCustomPost('/api/hr/employees/documents/upload/', [
        ['employee-my-requests'],
        ['hr-employee-dashboard'],
    ]);
    ...
    const confirmDeleteRequest = async () => {
        ...
        if (!req?.id) return;
        try {
            await deleteRequestMutation.mutateAsync({ id: req.id, request_type: req.request_type });
            toast.success('Request removed.');
            setDeleteConfirmRequest(null);
        } catch (e) {
            toast.error(e?.response?.data?.detail || e?.message || 'Could not remove request.');
        }
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            {showDocumentModal && selectedRequest && (
                ...
                >
                    <div
                        ...
                        style={{ width: 'min(900px, 100%)', maxHeight: '92vh', display: 'flex', flexDirection: 'column', gap: '1rem' }}
                    >
                        <Card
                        ...
                        >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                                <h3 id="document-preview-title" style={{ fontSize: '1.125rem', fontWeight: 700, margin: 0 }}>
                                    Document
                                </h3>
                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                    <Button
                                        ...
                                        onClick={() => handleDownloadDocument(selectedRequest.document_url)}
                                    >
                                        Download
                                    </Button>
                                    <Button
            ...
            <Card className="padding-none">
                ...
                {requests.length === 0 ? (
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                        ...
                        <tbody>
                            {requests.map((req) => {
                                const typeLabel = requestTypeLabel(req.request_type || req.type || req.leave_type);
                                const rawDate = req.date || req.requested_at || req.created_at || null;
                                ...
                                const isRejected =
                                    statusCode === 'rejected' || statusCode === 'declined' || statusCode === 'denied';
                                const requestKind = String(req.request_type || '').toLowerCase();
                                ...
                                if (requestKind === 'document') {
                                    ...
                                    if (isApproved) {
                                        actionsCell = (
                                            ...
                                            >
                                                This leave request was approved. No further action is required.
                                            </span>
                                        );
                                    } else if (isRejected) {
                                        actionsCell = (
                                            ...
                                            >
                                                This leave request was rejected. Contact HR if you need clarification.
                                            </span>
                                        );
                                    }
                                }
                                ...
                                return (
                                    <tr key={req.id || `${typeLabel}-${dateLabel}`} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                        <td style={{ padding: '1rem 1.5rem', fontWeight: 500 }}>{typeLabel}</td>
                                        <td style={{ padding: '1rem 1rem', color: 'var(--color-text-secondary)' }}>{dateLabel}</td>
            ...
            <ConfirmationModal
                ...
                cancelText="Cancel"
                onCancel={() => !deleteRequestMutation.isPending && setDeleteConfirmRequest(null)}
