...
function extractTimeTrackerStartPayload(data) {
    if (!data || typeof data !== 'object') return { lineId: null, startTime: null };
    const nested = data.data && typeof data.data === 'object' ? data.data : data;
    const lineId = nested.line_id ?? nested.lineId ?? nested.id ?? null;
    const startTime = nested.started_at ?? nested.start_time ?? nested.startTime ?? null;
    return {
...
const EmployeeDashboard = () => {
    ...
    const syncFromTodayPayload = useTimeTrackerStore((s) => s.syncFromTodayPayload);
    const dashboardQuery = useCustomQuery('/api/hr/employees/dashboard/', ['hr-employee-dashboard'], {
        staleTime: 1000 * 60 * 2,
    });
    const todayTrackerQuery = useCustomQuery('/api/hr/time-trackers/today/', ['hr-time-trackers-today'], {
        staleTime: 1000 * 30,
    });
    ...
    const createLeaveMutation = useCustomPost('/api/hr/employees/leaves/create/', [['hr-employee-dashboard']]);
    const uploadDocumentMutation = useCustomPost('/api/hr/employees/documents/upload/', [['hr-employee-dashboard']]);
    const startTimeTrackerMutation = useCustomPost('/api/hr/time-trackers/start/', [
    ...
    const projectSelectOptions = useMemo(
        () => assignmentProjects.map((p) => ({ value: String(p.id), label: p.name || 'Untitled project' })),
        [assignmentProjects]
    );
    const completedTodayLines = useMemo(() => {
        const lines = Array.isArray(todayTrackerQuery.data?.lines) ? todayTrackerQuery.data.lines : [];
        return lines
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            const { lineId, startTime: serverStart } = extractTimeTrackerStartPayload(res);
            start({
            ...
            toast.error('Could not start time tracker.', { description: formatTrackerMutationError(error) });
        }
    };

    const handleStop = async () => {
        if (activeActivity?.status !== 'running' || stopTimeTrackerMutation.isPending) return;
        ...
        try {
            ...
            toast.error('Could not stop time tracker.', { description: formatTrackerMutationError(error) });
        }
    };

    const getProjectName = (projectId) => {
        const key = String(projectId);
        const fromAssignments = assignmentProjects.find((p) => String(p.id) === key);
    ...
    const onSubmitRequest = async (values) => {
        try {
            ...
            const fallback =
                ...
                error?.message ||
                (values.request_kind === 'document' ? 'Could not upload document.' : 'Could not submit leave request.');

            const renderErrorDescription = (payload) => {
                if (!payload) return null;
                if (typeof payload === 'string') return <div>{payload}</div>;
                if (Array.isArray(payload)) {
                    const lines = payload.map((item) => (typeof item === 'string' ? item : JSON.stringify(item))).filter(Boolean);
                    if (lines.length === 0) return null;
                    return (
                        <div className="flex flex-col gap-1.5 text-start" style={{ fontSize: '0.8125rem', lineHeight: 1.45 }}>
                            {lines.map((line, i) => (
                                <div key={i}>{line}</div>
                            ))}
                        </div>
                ...
                if (typeof payload === 'object') {
                    ...
                    return (
                        <div className="flex flex-col gap-2 text-start" style={{ fontSize: '0.8125rem', lineHeight: 1.45 }}>
                            {entries.map(([key, value]) => {
                                ...
                                const cleaned = items.map((item) => String(item)).filter((item) => item.trim());
                                if (cleaned.length === 0) return null;

                                return (
                                    <div key={key} className="flex flex-col gap-1">
                                        <div style={{ fontWeight: 650, color: 'var(--color-text-main)' }}>
                                            {key.replace(/_/g, ' ')}
                                        </div>
            ...
            if (description) {
                toast.error(values.request_kind === 'document' ? 'Document upload failed' : 'Leave request failed', {
                    description,
                });
            } else {
    ...
    const isEmptyDashboard = dashboardQuery.isSuccess && (!dashboardQuery.data || Object.keys(dashboardQuery.data).length === 0);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            {showRequestModal && (
                <div style={{
                    ...
                    <Card className="padding-lg" style={{ width: '400px' }}>
                        ...
                        <form onSubmit={handleSubmit(onSubmitRequest)} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                <label style={{ fontSize: '0.875rem', fontWeight: 500 }}>Request</label>
                                <select
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                        ...
                                        >
                                            ...
                                            <span style={{ flex: 1, minWidth: 0 }}>
                                                <div style={{ fontWeight: 650, fontSize: '0.9rem', color: 'var(--color-text-main)' }}>
                                                    {selectedDocumentFile ? 'File selected' : 'Choose a file'}
                                                </div>
                                                <div
                                                    style={{
                                                        fontSize: '0.8rem',
                                                ...
                                                >
                                                    {selectedDocumentFile ? selectedDocumentFile.name : 'PDF, PNG, JPG (max size depends on server)'}
                                                </div>
                                            </span>
                                ...
                                <>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                        ...
                                        {errors.leave_type?.message && (
                                            <span style={{ fontSize: '0.75rem', color: 'var(--color-error)' }}>{errors.leave_type.message}</span>
                                        )}
                                    </div>
                                        ...
                                        <Input
                                            ...
                                            {...register('end_date', {
                                                ...
                                                validate: (value) => {
                                                    ...
                                                    return value >= leaveStartDate || 'End date must be after start date.';
                                                },
                                            })}
                                        />
                                    </div>
                                    <Input label="Days" type="number" value={computedLeaveDays} readOnly />
                                    <Input label="Notes" placeholder="Family travel" {...register('notes')} />
                                </>
                            )}
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', justifyContent: 'flex-end' }}>
                                ...
                                <Button
                                    type="submit"
                                    disabled={isSubmitting || (requestKind === 'leave' && computedLeaveDays <= 0)}
                                >
                                    {isSubmitting ? 'Submitting…' : requestKind === 'document' ? 'Upload Document' : 'Submit Leave'}
                                </Button>
                            </div>
                        </form>
                    </Card>
            ...
            <Card className="padding-lg" style={{
                border: activeActivity?.status === 'running' ? '2px solid var(--color-success)' : '1px solid var(--color-border)',
                background: activeActivity?.status === 'running' ? 'linear-gradient(to right, #f0fdf4, white)' : undefined
            }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                    <div style={{
                        padding: '0.5rem', borderRadius: 'var(--radius-md)',
                        background: activeActivity?.status === 'running' ? 'var(--color-success)' : 'var(--color-primary-50)',
                        color: activeActivity?.status === 'running' ? 'white' : 'var(--color-primary-600)'
                    }}>
                        <Timer size={20} />
                    </div>
                ...
                {activeActivity?.status === 'running' ? (
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'var(--color-slate-50)', borderRadius: '10px', marginBottom: '1rem' }}>
                            <div>
                                <div style={{ fontWeight: 600, fontSize: '0.95rem' }}>{getProjectName(activeActivity.projectId)}</div>
                                {activeActivity.description && (
                                    <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', marginTop: '0.25rem' }}>{activeActivity.description}</div>
                                )}
                                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: '0.25rem' }}>
                                    Started at {formatDateTimeSimple(activeActivity.startTime) || '—'}
                                </div>
                            </div>
                            <Button
                                variant="outline"
                            ...
                            >
                                <Square size={16} fill="var(--color-error)" /> {stopTimeTrackerMutation.isPending ? 'Stopping…' : 'Stop'}
                            </Button>
                        </div>
                    </div>
                ) : (
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
                        <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                            <SelectWithLoadMore
                                ...
                                paginationError={
                                    assignmentsQuery.isError ? 'Could not load projects. Scroll to retry or refresh the page.' : null
                                }
                                zIndex={500}
                            />
                        </div>
                        <div style={{ flex: '1.5 1 260px', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        <Button
                            onClick={handleStart}
                            disabled={!selectedProject || startTimeTrackerMutation.isPending || assignmentsQuery.isPending}
                            style={{
                        ...
                        >
                            <Play size={16} fill="white" /> {startTimeTrackerMutation.isPending ? 'Starting…' : 'Start'}
                        </Button>
                    </div>
                )}
                ...
                <div style={{ marginTop: '1.25rem', borderTop: '1px solid var(--color-border)', paddingTop: '1rem' }}>
                    ...
                    {todayTrackerQuery.isLoading && !todayTrackerQuery.data ? (
                        <div style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)' }}>Loading today&apos;s log…</div>
                        ...
                        completedTodayLines.map((line, index) => {
                            ...
                            const startedLabel = formatDateTimeSimple(line?.started_at) || '—';
                            return (
                                <div
                                ...
                                >
                                    <div>
                                        <div style={{ fontSize: '0.85rem', fontWeight: 500 }}>{projectName}</div>
                                        {line.note ? (
                                            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)' }}>{line.note}</div>
                                        ) : null}
                                    </div>
                                    <div style={{ textAlign: 'right' }}>
                                        <div style={{ fontSize: '0.85rem', fontWeight: 600, fontFamily: 'monospace' }}>
                                            {formatDuration(Number.isFinite(durationSec) ? durationSec : 0)}
                                        </div>
                                        <div style={{ fontSize: '0.7rem', color: 'var(--color-text-muted)' }}>{startedLabel}</div>
                                    </div>
                                </div>
                            );
                        })
                        ...
                        {latestPayslip ? (
                            <>
                                <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>
                                    {formatCurrency(latestPayslip.amount, latestPayslip.currency || 'USD')}
                                </div>
                        ...
                        {upcomingHolidays.length === 0 ? (
                            ...
                            upcomingHolidays.map((holiday, index) => (
                                ...
                                >
                                    ...
                                    <span style={{ color: 'var(--color-text-muted)' }}>{formatDate(holiday.date) || '-'}</span>
                                </li>
                            ))
                        )}
                    </ul>
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                            ...
                            <tbody>
                                {myRequests.map(req => {
                                    ...
                                    const requestType = req.type || req.leave_type || req.request_type || 'Request';
                                    ...
                                    return (
                                        <tr key={req.id || `${req.type}-${req.created_at}`} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                            <td style={{ padding: '1rem 1.5rem', fontWeight: 500 }}>{requestType}</td>
                                            ...
                                            <td style={{ padding: '1rem 1rem', color: 'var(--color-text-secondary)', fontWeight: 600 }}>
                                                {requestDays != null && requestDays !== '' ? requestDays : '-'}
                                            </td>
