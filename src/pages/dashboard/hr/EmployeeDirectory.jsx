...
const EmployeeDirectory = () => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '1.5rem' }}>
                ...
                <Card className="padding-md" style={{ borderLeft: '4px solid var(--color-success-500)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                        <div style={{ padding: '0.75rem', background: 'var(--color-slate-50)', borderRadius: 'var(--radius-md)' }}>
                        ...
                        <div style={{ padding: '0.75rem', background: 'var(--color-success-50)', borderRadius: 'var(--radius-md)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '120px', overflowY: 'auto' }}>
                            {payrollByDept.slice(0, 4).map((dept, idx) => (
                                <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.8rem' }}>
                                    <span style={{ width: '35%', color: 'var(--color-text-primary)' }}>{dept.name}</span>
                                    <div style={{ flex: 1, background: 'var(--color-slate-100)', height: '6px', borderRadius: '3px', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem', background: 'white', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <select
                    style={{ padding: '0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', fontSize: '0.875rem', minWidth: '160px' }}
                    value={filterDept}
                ...
                <select
                    style={{ padding: '0.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', fontSize: '0.875rem', minWidth: '140px' }}
                    value={filterStatus}
                ...
                {filteredEmployees.map((emp) => (
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
                            <div style={{ width: '3rem', height: '3rem', borderRadius: '50%', background: 'var(--color-primary-100)', color: 'var(--color-primary-700)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600 }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                            <span style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem', background: 'var(--color-slate-100)', borderRadius: '1rem' }}>{getDeptName(emp.departmentId)}</span>
                            <span style={{ fontSize: '0.75rem', padding: '0.25rem 0.5rem', background: emp.status === 'Active' ? 'var(--color-success-bg)' : 'var(--color-slate-100)', color: emp.status === 'Active' ? 'var(--color-success)' : 'inherit', borderRadius: '1rem' }}>{emp.status}</span>
                        </div>
