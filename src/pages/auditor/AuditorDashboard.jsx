...
const AuditorDashboard = () => {
  ...
  const getCompany = (companyId) =>
    clientCompanies.find((c) => c.id === companyId);
...
═══════════════════════════════════════════════════════`.trim();
    ...
    a.href = url;
    a.download = `sealed_${period.label.replace(/\s/g, "_")}_v${period.version}.txt`;
    a.click();
  ...
  if (viewMode === "review" && selectedPeriod) {
    const isNewVersion = [
      AUDIT_STATUSES.APPROVED,
      AUDIT_STATUSES.SEALED,
    ].includes(selectedPeriod.status);
    return (
      <>
        <div
          style={{
          ...
          <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
                ...
                <Card className="padding-md">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <strong>{postedEntries.length}</strong>
                    </div>
                    <div>
                      <span style={{ color: "var(--color-text-muted)" }}>
                        Invoices:
                      </span>{" "}
                      <strong>{invoices?.length || 0}</strong>
                    </div>
                    <div>
                      <span style={{ color: "var(--color-text-muted)" }}>
                        Bank Accounts:
            ...
            {activeAccountingTab === "journal" && (
              <Card className="padding-none">
                ...
                >
                  <h4 style={{ fontWeight: 700 }}>
                    Journal Entries ({postedEntries.length} posted)
                  </h4>
                  <Button
                    size="sm"
                    icon={<Plus size={14} />}
                    onClick={() => {
                      ...
                      if (desc && acc1 && acc2 && amount > 0) {
                        addEntry({
                          ...
                          reference: `ADT-${Date.now().toString().slice(-6)}`,
                          description: `[Auditor] ${desc}`,
                          status: "Posted",
                          isAutomatic: false,
                          sourceType: "Auditor Adjustment",
                          lines: [
                            {
                              id: 1,
                              account: acc1,
                              ...
                              setEditModal({
                                ...
                                fields: [
                                  ...
                                  ...entry.lines.map((line, i) => ({
                                    key: `debit_${i}`,
                                    label: `${line.account} - Debit`,
                                    value: line.debit,
                                    oldValue: String(line.debit),
                                    type: "number",
                                  })),
                                  ...entry.lines.map((line, i) => ({
                                    key: `credit_${i}`,
                                    label: `${line.account} - Credit`,
                                    value: line.credit,
                                    oldValue: String(line.credit),
                                    type: "number",
                                  })),
                                ],
                                onSave: (vals) => {
                                  ...
                                  entry.lines.forEach((line, i) => {
                                    const newDebit =
                                      parseFloat(vals[`debit_${i}`]) || 0;
                                    const newCredit =
                                      parseFloat(vals[`credit_${i}`]) || 0;
                                    if (newDebit !== line.debit)
                                      logChange({
                                        entityType: "journal_line",
                                        entityId: entry.id,
                                        field: `debit (${line.account})`,
                                        oldValue: String(line.debit),
                                        newValue: String(newDebit),
                                        periodId: selectedPeriod?.id,
                                        companyId: selectedPeriod?.companyId,
                                      });
                                    if (newCredit !== line.credit)
                                      logChange({
                                        entityType: "journal_line",
                                        entityId: entry.id,
                                        field: `credit (${line.account})`,
                                        oldValue: String(line.credit),
                                        newValue: String(newCredit),
                                        periodId: selectedPeriod?.id,
                                        companyId: selectedPeriod?.companyId,
                                      });
                                  });
                  ...
                  >
                    ...
                    >
                      <div>
                        ...
                        >
                          Propose accounting adjustments that require admin
                          approval.
                        </p>
                      </div>
                      <Button
                      ...
                      >
                        ...
                        <div style={{ marginBottom: "1rem" }}>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setNewAdjDescription(e.target.value)
                            }
                          ...
                          >
                            <thead>
                              <tr style={{ background: "white" }}>
                                ...
                                <th
                                  style={{ padding: "6px 8px", width: "40px" }}
                                ></th>
                              </tr>
                            </thead>
                            <tbody>
                        ...
                        >
                          <Button
                            variant="ghost"
                            onClick={() => setShowNewAdjForm(false)}
                          >
                            Cancel
                          </Button>
                          <Button
                    ...
                    {periodAdjs.length === 0 && !showNewAdjForm ? (
                      ...
                      >
                        ...
                        >
                          Click "New Adjustment" to propose an accounting
                          change.
                        </p>
                      </Card>
                    ) : (
            ...
            {(() => {
              const myChanges = auditChanges.filter(
                (c) => c.companyId === selectedPeriod?.companyId,
              );
              if (myChanges.length === 0) return null;
              const pendingCount = myChanges.filter(
                (c) => c.status === "pending",
              ).length;
              const approvedCount = myChanges.filter(
                (c) => c.status === "approved",
              ).length;
              const rejectedCount = myChanges.filter(
                (c) => c.status === "rejected",
              ).length;
              return (
                <Card
